...
pin_model_data <- function(board, data, name, title = NULL, description = NULL, ...) {
  ...
  model_data_name <- pins::pin_write(board,
                               data, # TODO confirm this is the model object and change the name
...
insert_model_run <- function(con, model, blob_url_list, description, results_name, inputs){
  ...
  model_final_results$model_name <- tolower(model_final_results$model_name) # TODO we are no longer extracting parameters so model final results will not exist. Can we pull the model name from somewhere else?
