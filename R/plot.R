...
plot_pCap_all_sites <- function(
  ...
  obs_vs_pred_plot <- ggplot2::ggplot(
    scatter_df,
    ggplot2::aes(x = obs, y = pred)
  ) +
  ...
  hist_plot <- ggplot2::ggplot(
    hist_df,
    ggplot2::aes(x = value, fill = source)
  ) +
    ...
    ggplot2::geom_vline(
      data = mean_df,
      ggplot2::aes(xintercept = xint, colour = source, linetype = source),
      linewidth = 0.8
    ) +
...
plot_pCap_main <- function(
  ...
  dp <- as.data.frame(
    pcap,
    pars = c("logit_pCap", "b0_pCap", "b_flow", "pro_sd_P", "alpha")
  )
...
plot_pCap_site_year <- function(
  ...
  abundance_plot <- ggplot2::ggplot(abund_df, ggplot2::aes(x = date_label)) +
    ggplot2::geom_bar(
    ...
    ggplot2::scale_y_continuous(
      ...
      sec.axis = ggplot2::sec_axis(
        transform = ~ flow_range[1] +
          (. - N_range[1]) / diff(N_range) * diff(flow_range),
        name = "Discharge (kcfs)"
      )
    ) +
    ggplot2::labs(
  ...
  for (i in seq_len(Nstrata)) {
    ...
    if (length(irecs_min) == 0) {
      irecs_min <- seq_len(nrow(dp_pcap))
    }
  ...
  pcap_plot <- ggplot2::ggplot(pcap_df, ggplot2::aes(x = date_label)) +
    ...
    ggplot2::scale_y_continuous(
      ...
      sec.axis = ggplot2::sec_axis(
        transform = ~ flow_range[1] +
          (. - pCap_range[1]) / diff(pCap_range) * diff(flow_range),
        name = "Discharge (kcfs)"
      )
    ) +
    ggplot2::labs(
...
#'
#' @export
plot_year_re_with_effort <- function(
  pcap,
  ...
  if (model_type == "all_sites") {
    ...
    dp <- as.data.frame(
      pcap,
      pars = c("b0_pCap", "yr_re", "yr_sd_P", "pro_sd_P", "alpha")
    )
  }
  ...
  yr_re_plot <- ggplot2::ggplot(yr_re_df, ggplot2::aes(x = idx, y = mean_re)) +
    ggplot2::geom_vline(
  ...
  site_year_plot <- ggplot2::ggplot(
    ...
    ggplot2::geom_errorbar(
      ggplot2::aes(ymin = lo_pCap, ymax = hi_pCap, colour = is_no_et),
      width = 0.25
    ) +
    ggplot2::geom_point(
...
generate_diagnostic_plot_juv <- function(inputs, results_df) {
  ...
  pCap_estimates <- params |>
    ...
    select(week = week_fit, c("97.5", "50", "mean", "75", "sd", "25", "2.5"))
  ...
  abundance_plot <- N_estimates |>
    left_join(inputs$lp_data, by = "week") |>
    ...
    geom_point(
      aes(x = date, y = lincoln_peterson_abundance),
      shape = 1,
      color = "blue",
      size = 3
    ) +
    geom_point(aes(x = date, y = Inf, color = sampled), size = 3) +
    geom_text(
      aes(x = date, y = Inf, label = paste(count_label), angle = 90),
      hjust = 1,
      size = 3
    ) +
  ...
  efficiency_plot <- pCap_estimates |>
    left_join(inputs$lp_data, by = "week") |>
    mutate(
      across(c(mean, `50`, `2.5`, `97.5`), plogis),
      number_released_label = ifelse(
        is.na(number_released),
        "",
        number_released
      ),
      number_recaptured_label = ifelse(
    ...
    geom_errorbar(aes(x = date, ymin = `2.5`, ymax = `97.5`), width = 0.2) +
    geom_point(
      aes(x = date, y = lincoln_peterson_efficiency),
      shape = 1,
      color = "blue",
      size = 3
    ) +
...
plot_juv_data <- function(site, run_year) {
  data <- SRJPEdata::weekly_juvenile_abundance_catch_data |>
    ...
    arrange(ns_order) |>
    select(-ns_order) |>
    mutate(date = factor(date, levels = date), week_index = row_number())

  abundance_plot <- data |>
    ggplot(aes(x = date, y = count)) +
    geom_bar(stat = "identity", fill = "grey", width = .75) +
    geom_text(
      aes(x = date, y = Inf, label = paste(count), angle = 90),
      hjust = 1,
      size = 3
    ) +
    theme_minimal() +
