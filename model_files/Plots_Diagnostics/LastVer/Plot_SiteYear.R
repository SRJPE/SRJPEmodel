...
if(RunMainstem==F){
  ...
  outdir="C:/Projects/BayDelta/SAC_JPE/SRJPEmodel/model_files/output/trib"
  pdf(file=paste0(outdir,"/trib_all.pdf"))
  pCap_inputs <- prepare_pCap_inputs(mainstem=RunMainstem) #do this here to not load it for each tributayr

} else {
  d0=subset(d0a,site=="knights landing" | site=="tisdale")
  outdir="C:/Projects/BayDelta/SAC_JPE/SRJPEmodel/model_files/output/mainstem"
  pdf(file=paste0(outdir,"/main_all.pdf"))
}
