...
for (ii in 1:3) {
  #1:4
  if (ii == 1) {
    description <- paste0("pCap tributary sites test ", Sys.Date())
...
pcap_knights_landing <- get_model_fit(
  "pcap_one_site",
  con = con,
  site_selection = "knights landing"
)
pcap_all_sites <- get_model_fit("pcap_all_sites", con = con)

for (i in 1:nrow(combinations_to_fit)) {
