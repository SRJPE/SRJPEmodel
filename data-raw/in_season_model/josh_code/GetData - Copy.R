...
fn1=list.files(path="../../RST/RunSize/Output",pattern=doTrib)
iuse=grepl(pattern="sum",x=fn1)
fn2=fn1[iuse]
Nyrs=length(fn2)
...
dQ0=read.csv(file="../../RST/data/standard_flow.csv",header=T)
Csite=sub(".*\\_","",doTrib)#the site name in dQ0 data frame
dQ=subset(dQ0,site==Csite)

#par(mfrow=c(4,4),mai=c(0.3,0.2,0.3,0.2), omi=c(0.5,0.5,0.5,0.5),cex.main=1.1,cex.lab=1.2,font.lab=2)
for(iyr in 1:Nyrs){
  x=unlist(gregexpr('_sum', fn2[iyr]))
  year[iyr]=as.double(substr(fn2[iyr],start=x-4,stop=x-1))
  
  fn_sum=paste0("../../RST/RunSize/Output/",doTrib,"_",year[iyr],"_sum.out")  
  dp=read.table(file=fn_sum,header=T)
  
  irow=which(rownames(dp)=="Ntot")
  Ntot_cv[iyr]=dp$sd[irow]/dp$mean[irow] #for likelihood in log space
  Ntot_mu[iyr]=log(dp$mean[irow])
  Ntot_sd[iyr]= sqrt(log(Ntot_cv[iyr]^2+1))
  
  fn_dat=paste0("../../RST/RunSize/Output/",doTrib,"_",year[iyr],"_data.out")  
  dd=read.table(file=fn_dat,header=T)
  
  TrimRun=F
  #if(doTrib=="battle creek_ubc" & (year[iyr]==2005|year[iyr]==2006|year[iyr]==2017|year[iyr]==2018|year[iyr]==2021)) TrimRun=T
  #if(doTrib=="clear creek_lcc" & (year[iyr]==2005|year[iyr]==2006|year[iyr]==2018)) TrimRun=T
  
  #strip out some of initial and last weeks with 0 catch
  if(TrimRun==T){
    wkbuff=3
    fuwk=min(which(dd$u>0))-wkbuff;  if(fuwk<=0) fuwk=1
    luwk=max(which(dd$u>0))+wkbuff;if(luwk>dim(dd)[1]) luwk=dim(dd)[1]
    #luwk=dim(dd)[1]
    Nestwks[iyr]=luwk-fuwk+1
  } else { #Use all weeks where abundance was estimated
    fuwk=1
    Nestwks[iyr]=dim(dd)[1]
  }
  
  #first modelled week is 35 which should be at end of previous year. Data error
  #if(doTrib=="battle creek_ubc" & year[iyr]==2006){
  
  frow=which(rownames(dp)==paste0("N[",fuwk,"]"))
  for(iwk in 1:Nestwks[iyr]){
    ewk[iwk,iyr]=which(mwk==dd$Jwk[iwk+fuwk-1])
    pwk[iwk,iyr]=theta[ewk[iwk,iyr]]
   
    lrow=which(rownames(dp)==paste0("N[",iwk+fuwk-1,"]"))
    irows=frow:lrow #the list of rows for abundance from first week through current week
    
    mu=sum(dp$mean[irows])#mean abundance from start of sampling through week iwk
    ...
    varx=sum(dp$sd[irows]^2)#total variance across weekly estimates is sum of the weekly variances
    if(varx==0) varx=0.01 #can also happen if multiple weeks of 0 catch at start of sampling
    Nx_cv[iwk,iyr]=sqrt(varx)/mu#the cv in untranformed space
  ...
  dQ1=subset(dQ,as.integer(substr(dQ$date,1,4))==year[iyr]-1) #records in calender year prior to current run year (e.g., Oct-Dec)
  dQ2=subset(dQ,as.integer(substr(dQ$date,1,4))==year[iyr]) #records from current year (e.g., Jan-May)
  dQ3=rbind(dQ1,dQ2) #combine
  #select records for window to calculate peak Q in this example
  irecs=which(dQ3$date >=paste0(year[iyr]-1,"-10-01") & dQ3$date <= paste0(year[iyr],"-01-31"))
  CovX0[iyr,1]=max(dQ3$flow_cfs[irecs])
