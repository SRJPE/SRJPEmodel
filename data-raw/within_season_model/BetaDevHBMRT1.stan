...
parameters {
 ...
 array[Nyrs] real<lower=0.1> sd_pro;//sd for random effects
...
model {
  ...
  for(iyr in 1:Nyrs){
    ...
    sd_pro[iyr]~gamma(hyp_sd_pro[1],hyp_sd_pro[2]);//annual sd_pro values are random deviates from gamma hyper distribution
...
generated quantities{
  ...
  //real ssd_pro=mean(sd_pro);//temporary until a hyper for sd_pro (across year) is estimated
  real ssd_pro=gamma_rng(hyp_sd_pro[1],hyp_sd_pro[2]);
  
  //predict the proportion of run passing by week based on simulated values beta values and simulated eviations
