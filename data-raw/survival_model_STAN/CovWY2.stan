...
//This version uses year-reach specific pCaps by water year fixed effect (constant across reaches)
...
transformed parameters{
  ...
  for(i in 1:Nind){
    for(j in 1:Ndetlocs){  
      if(j<Ndetlocs) surv[i,j]=inv_logit(S_bReach[rch_covind[j]] + S_bCov*CovX[i] + S_RE[rgind[i]])^Rmult[j];
...
generated quantities{
  ...
  for(irg in 1:Nrg){
    for(j in 1:Nreaches){
      ...
      if(rgwy_ind[irg]==0){
        ...
        pred_surv[irg,j]=inv_logit(S_bReach[rch_covind[j]] +  S_bCov + S_RE[irg])^Rmult[j];
      }
  ...
  SurvForecast[2]=inv_logit(S_bReach[2] +  S_bCov + s_re)^Rmult[2] * inv_logit(S_bReach[3] +  S_bCov + s_re)^Rmult[3];  //wet year
}
