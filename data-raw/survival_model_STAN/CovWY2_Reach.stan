...
//This version uses year-reach specific pCaps but survival influenced by a wet year fixed effect (varies across reaches with last 2 using same value)
...
parameters {
  ...
  vector <lower=-10,upper=10> [Nreaches-1] S_bCov;
...
transformed parameters{
  ...
  for(i in 1:Nind){
    for(j in 1:Ndetlocs){  
      if(j<Ndetlocs) surv[i,j]=inv_logit(S_bReach[rch_covind[j]] + S_bCov[rch_covind[j]]*CovX[i] + S_RE[rgind[i]])^Rmult[j];
  ...
  for(irg in 1:Nrg){
    for(j in 1:Nreaches){
      if(rgwy_ind[irg]==0){
        ...
        pred_surv[irg,j]=inv_logit(S_bReach[rch_covind[j]] +  S_bCov[rch_covind[j]] + S_RE[irg])^Rmult[j];
      }
    }
    SurvWoodSac[irg]=pred_surv[irg,2]*pred_surv[irg,3]; 
  }
 
  vector[2] SurvForecast;
  real s_re=normal_rng(0,RE_sd);
  SurvForecast[1]=inv_logit(S_bReach[2] +  s_re) ^Rmult[2] * inv_logit(S_bReach[3] +  s_re)^Rmult[3];  //dry year
  SurvForecast[2]=inv_logit(S_bReach[2] +  S_bCov[2] + s_re)^Rmult[2] * inv_logit(S_bReach[3] +  S_bCov[3] + s_re)^Rmult[3];  //wet year
