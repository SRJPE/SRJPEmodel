...
//as well as size-based individual effect that doesn't vary across reaches.
data {
  int Nind;
  int Nreaches;
  int Ndetlocs;
...
transformed parameters{
  ...
  for(i in 1:Nind){
    for(j in 1:Ndetlocs){  
      if(j<Ndetlocs) {
        surv[i,j]=inv_logit(S_bReach[rch_covind[j]] + S_bCov*CovX[i,j] + S_bSz*Sz[i] + S_RE[rgind[i]])^Rmult[j];
      }
...
generated quantities{
  ...
  array[Nseq] vector[Nreaches] pred_survSz;
  vector[Nseq] SurvWoodSac;
  vector[Nseq] SurvWoodSacSz;
  for(ix in 1:Nseq){
    for(j in 1:Nreaches){
         pred_surv[ix,j]=inv_logit(S_bReach[rch_covind[j]] +  S_bCov*Xseq[ix])^Rmult[j];
         pred_survSz[ix,j]=inv_logit(S_bReach[rch_covind[j]] +  S_bSz*Xsz[ix])^Rmult[j];
     }
    SurvWoodSac[ix]=pred_surv[ix,2]*pred_surv[ix,3]; 
    SurvWoodSacSz[ix]=pred_survSz[ix,2]*pred_survSz[ix,3]; 
  }
  
  vector[Nseq] SurvForecast;
  vector[Nseq] SurvForecastSz;
  ...
  for(ix in 1:Nseq){
    ...
    SurvForecastSz[ix]=inv_logit(S_bReach[2] +  S_bSz*Xsz[ix] + s_re)^Rmult[2] * inv_logit(S_bReach[3] +  S_bSz*Xsz[ix] + s_re)^Rmult[3]; 
  }
