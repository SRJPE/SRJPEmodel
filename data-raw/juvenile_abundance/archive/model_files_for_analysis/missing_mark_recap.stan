...
model {
  ...
  b_sp[1] ~ normal(0, 1);
  b_sp[2] ~ normal(0, 1);
  //b_sp[2] ~ uniform(-10, 10); // this was dflat() and may need to be tweaked (chatGPT said normal(0, 1))
  for (i in 3:K) {
    real xi = 2 * b_sp[i-1] - b_sp[i-2];
    b_sp[i] ~ normal(xi, tau_N);
  }

  // simulated process error deviations, pulling from normal distribution
  for(i in 1:Nwomr){
  ...
  for(i in 1:Nstrata){
    lg_N[i] ~ normal(Usp[i], sd_Ne); // temporary
    //lg_N[i] ~ normal_lpdf(lgN_max[i] | Usp[i], sd_Ne); // need to tweak this?
    //lg_N[i] = fmin(Usp[i] + normal_rng(0, sd_Ne), lgN_max[i]); // truncated normal
  }

  // Likelihood for unmarked catch observations
  //int N_int[Nstrata] = round(N);
  for (i in 1:Nstrata_wc) {
    u[Uwc_ind[i]] ~ poisson(pCap_U[Uwc_ind[i]] * N[Uwc_ind[i]]); // temporary to get past integer issue
    //u[Uwc_ind[i]] ~ binomial(N_int[Uwc_ind[i]], pCap_U[Uwc_ind[i]]);
  }
}
