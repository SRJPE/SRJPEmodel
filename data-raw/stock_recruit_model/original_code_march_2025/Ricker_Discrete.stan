...
data {
  ...
  array[Nyrs] int X;//water year type as an index (0, 1, 2,...)
  int Ndisc;// number of discrete effects to be estimated
}

parameters {
 real alpha;
 real <upper=0> beta;//can't have a positive beta (higher spawning stock = higher survival)
 array[Ndisc] real <lower=-5,upper=5> gamma;
...
transformed parameters{
  ...
  for(iyr in 1:Nyrs){
    if(X[iyr]==0){
      lgRS[iyr] = alpha + beta*SP[iyr] + dev[iyr];
    } else {
...
model {
  ...
  alpha~normal(0,1000);
  beta~normal(0,1000);
  for(i in 1:Ndisc){gamma[i]~normal(0,1000);}
}
