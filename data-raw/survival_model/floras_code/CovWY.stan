...
transformed parameters{
  ...
  for(i in 1:Nind){
    for(j in 1:Ndetlocs){  
      if(j<Ndetlocs) {
        if(CovX[i]==0){
          surv[i,j]=inv_logit(S_bReach[rch_covind[j]] + UseSizeEffect*S_bSz*Sz[i] + S_RE[rgind[i]])^Rmult[i,j];
        } else { 
          surv[i,j]=inv_logit(S_bReach[rch_covind[j]] + S_bCov[CovX[i]] + UseSizeEffect*S_bSz*Sz[i] + S_RE[rgind[i]])^Rmult[i,j];
        }
      }
  ...
  for(i in 1:NindT){
    ...
    if(CovXT[i]==0){
      survT[i,1]=inv_logit(S_bTrib[trib_ind[i]] + UseSizeEffect*S_bSz*SzT[i] + S_REt[rgindT[i]])^RmultT[i];
    } else {
      survT[i,1]=inv_logit(S_bTrib[trib_ind[i]] + S_bCovT[CovXT[i]] + UseSizeEffect*S_bSz*SzT[i] + S_REt[rgindT[i]])^RmultT[i];//RmultT is distance from release point to Sac station
    }
    //survival rate in Sac-Delta reach of mainstem
    int j=Nreaches;
    if(CovXT[i]==0){
      survT[i,2]=inv_logit(S_bReach[rch_covind[j]] + UseSizeEffect*S_bSz*SzT[i] + S_REt[rgindT[i]])^Rmult[1,j];//. 
    }else {
      survT[i,2]=inv_logit(S_bReach[rch_covind[j]] + S_bCov[CovXT[i]] + UseSizeEffect*S_bSz*SzT[i] + S_REt[rgindT[i]])^Rmult[1,j];//Same fixed effect for mainstem and tributary fish in mainstem 
    }
...
generated quantities{
  ...
  if(UseSizeEffect == 1){
    for(ix in 1:Nsz){
      SurvWoodSacSz[ix,1]=inv_logit(S_bReach[2] + S_bSz*Xsz[ix])^Rmult[1,2] * inv_logit(S_bReach[3] + S_bSz*Xsz[ix])^Rmult[1,3];  
      SurvRelSacSz[ix,1]=inv_logit(S_bReach[1] + S_bSz*Xsz[ix] )^Rmult[1,1] * inv_logit(S_bReach[2] + S_bSz*Xsz[ix] )^Rmult[1,2] * inv_logit(S_bReach[3] +  S_bSz*Xsz[ix])^Rmult[1,3];  
      
      for(j in 1:NS_bCov){
        SurvWoodSacSz[ix,j+1]=inv_logit(S_bReach[2] + S_bCov[j] + S_bSz*Xsz[ix])^Rmult[1,2] * inv_logit(S_bReach[3] + S_bCov[j] + S_bSz*Xsz[ix])^Rmult[1,3]; 
        SurvRelSacSz[ix,j+1]= inv_logit(S_bReach[1] + S_bCov[j] + S_bSz*Xsz[ix] )^Rmult[1,1] *  inv_logit(S_bReach[2] + S_bCov[j] + S_bSz*Xsz[ix])^Rmult[1,2] * inv_logit(S_bReach[3] + S_bCov[j] + S_bSz*Xsz[ix])^Rmult[1,3];  
      }
    }
  }
  ...
  if(UseSizeEffect == 1){
    for(ix in 1:Nsz){
      SurvForecastSz[ix,1]= inv_logit(S_bReach[1] + S_bSz*Xsz[ix] + s_re)^RmultSac * inv_logit(S_bReach[2] + S_bSz*Xsz[ix] + s_re)^Rmult[1,2] *
      inv_logit(S_bReach[3] +  S_bSz*Xsz[ix] + s_re)^Rmult[1,3];
      for(j in 1:NS_bCov){
        SurvForecastSz[ix,j+1]= inv_logit(S_bReach[1] +  S_bCov[j] + S_bSz*Xsz[ix] + s_re)^RmultSac * inv_logit(S_bReach[2] + S_bCov[j] + S_bSz*Xsz[ix] + s_re)^Rmult[1,2] *
        inv_logit(S_bReach[3] +  S_bCov[j] + S_bSz*Xsz[ix] + s_re)^Rmult[1,3];
