...
# Read data -----------------------------------------------------------------------
## Inp data
d_Sac <- read.csv(here('data','SacInp_2025.csv'),  stringsAsFactors = F)
...
# FPTflow <- dataRetrieval::readNWISdv(11447650, "00060", startDate = "2013-01-01",endDate="2024-12-31") %>% 
#   mutate(flow = as.numeric(X_00060_00003),
...
d_FeaBut_sort <- d_FeaBut %>% 
  ...
  mutate(lastCap = case_when(ch == 100 ~ 1,
         ...
         FlowexceedT = case_when(flowexceedtype =="Dry" ~ 0,
                                flowexceedtype == "Average" ~ 1,
                                flowexceedtype == "Wet" ~ 2)) %>% 
         # TempRel = case_when(rl %in% c('1B','2B','3B','4B','5B','6B') ~ tempButte.z,
         #                     TRUE ~ tempFeather.z)) %>% # standardize distances per 100km
  ungroup() %>% 
  mutate(Maxflow.z = scale(Maxflow))
