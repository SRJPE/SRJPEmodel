...
data {
 ...
 // array[Nind,Nreaches] real X;
//  array[NindT,NreachesT] real XT;
  array[NsX] real Xvec;
...
transformed parameters{
  ...
  for(i in 1:NindT){
    //survival and travel time rates in tributary until SAC detection location
     survT[i,1]=inv_logit(S_bTrib[trib_ind[i]] + S_bCovT*CovXT[i,1] + UseSizeEffect*S_bSz*SzT[i]+ S_REt[rgindT[i],1])^RmultT[i];//RmultT is distance from release point to Sac station
     TTT = exp(T_bTrib[trib_ind[i]] + T_bSz*SzT[i] + TT_bCovT *CovXT[i,1] + TT_RET[rgindT[i],1]);
...
generated quantities{
 ...
 vector[Nreaches] B0;
 B0 = exp(T_bReach); //Travel days per 100 km. Needed for output only
 ...
 array[Nind] vector[Nreaches] TT_reach;
  for(i in 1:Nind){
    ...
    pred_surv[i,1]=inv_logit(S_bReach[rch_covind[1]] + S_bCov*CovX[i,1]+ S_RE[rgind[i],1])^RmultSac;
    pred_surv_per100[i,1]=inv_logit(S_bReach[rch_covind[1]] +  S_bCov*CovX[i,1] + S_RE[rgind[i],1]);

    for(j in 2:Nreaches){ 
     pred_surv[i,j]=inv_logit(S_bReach[rch_covind[j]] +  S_bCov*CovX[i,j] + S_RE[rgind[i],j])^Rmult[1,j];
     TT_reach[i,j]= exp(T_bReach[rch_covind[j]] + TT_bCov*CovX[i,j] + TT_RE[rgind[i],j])*ReachKM_ind[i,j]/100;
     pred_surv_per100[i,j]=inv_logit(S_bReach[rch_covind[j]] +  S_bCov*CovX[i,j] + S_RE[rgind[i],j]);
    }
...
//vector[2] B0T;
 //B0T=exp(T_bTrib[,1]); //Travel days per 100 km. Needed for output only
// vector[NindT] TT_RelSacT;
 array[NindT] vector[2] TT_reachT;
  for(i in 1:NindT){
     pred_survT[i,1]=inv_logit(S_bTrib[trib_ind[i]] + S_bCovT*CovXT[i,1] + S_REt[rgindT[i],1])^RmultTrib[trib_ind[i]];//survival rate from release to Sac station
     pred_survT[i,2]=inv_logit(S_bReach[Nreaches]  + S_bCov*CovXT[i,2] + S_REt[rgindT[i],2] )^Rmult[1,Nreaches];//survival rate from Sac-Delta stations
    
     pred_survT_per100[i,1]=inv_logit(S_bTrib[trib_ind[i]] + S_bCovT*CovXT[i,1] + S_REt[rgindT[i],1]);//survival rate from release to Sac station
     pred_survT_per100[i,2]=inv_logit(S_bReach[Nreaches] + S_bCov*CovXT[i,2] + S_REt[rgindT[i],2] );//survival rate from Sac-Delta stations

//     TT_RelSacT[i]= exp(lgB0T + TT_bCovT *XT[i] +TT_RET[rgindT[i]])*sum(ReachKMT_ind[i,1:2])/100;
     TT_reachT[i,1]= exp(T_bTrib[trib_ind[i]] + TT_bCovT *CovXT[i,1] +TT_RET[rgindT[i],1])*ReachKMT_ind[i,1]/100;
...
for(itrib in 1:Ntribs){
  for(j in 1:NsX){
        ...
        TTForecastT[j,itrib] = exp(T_bTrib[itrib] + TT_bCovT *XvecT[j] + tt_reT)*ReachKMT[itrib,1]/100;
    }
  }
    ...
    for(ix in 1:Nsz){
        for(j in 1:NsX){
            for(itrib in 1:Ntribs){
               ...
               TTForecastTSz[ix,j,itrib] = exp(T_bTrib[itrib] + TT_bCovT *XvecT[j] + T_bSz*Xsz[ix]+ tt_reT)*ReachKMT[itrib,1]/100;
            }
               for(i in 1:NrstT){
                  TTForecastTSz_rst[ix,j,i] = exp(T_bTrib[TribForRST[i]] + TT_bCovT *XvecT[j] + T_bSz*Xsz[ix]+ tt_reT)*ReachKMTrst[i]/100;
