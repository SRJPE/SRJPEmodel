...
#' @param covariate either `no_covariate`, `water_year_2`, `water_year_3`, `water_year_exceedence`,
#' `maximum_flow`, `relative_temperature`, `date_of_release`, or `only_biological_effect`
...
#'
#' @md
prepare_survival_inputs <- function(covariate = NULL,
                                    effect = NULL){

  # check args
  if(is.null(covariate) & !is.null(effect)) {
    cli::cli_abort("If you would like to run a model with only a biological effect, please specify covariate as `only_biological_effect`.")
  }
  ...
  # TODO new if/else workflow
  # combination of covariate selection and effect
  # first, set model name based on covariate selection
  # then extract relevant data objects
  ...
  if(is.null(number_of_water_year_types) & null_effect) {
    ...
    # TODO add exceedence, max flow, dor
...
get_survival_data_list <- function(version, sac_data, feather_butte_data) {
  ...
  data_list <- list(n_ind = nrow(data),
                    ...
                    flow_exceed_index = flow_exceed_index,
                    max_flow_index = max_flow_index,
