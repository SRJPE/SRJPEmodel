...
write.csv(TTfinalSac_df ,here("outputs", "TravTime_Sac.csv"), row.names=FALSE)
...
write.csv(summary_TT_final ,here("outputs", "TravTime_Sac_summary.csv"), row.names=FALSE)
...
d_DHFeaBut <- data.frame(rbind(d_DHFea,d_DHBut)) %>% 
              arrange(FishID)
...
ReachKMT <- data.frame(rbind(cbind(92,110),cbind(117,110)))
...
write.csv(TTfinalFeaBut_df ,here("outputs", "TravTime_FeaBut.csv"), row.names=FALSE)
...
write.csv(summary_TTT_final ,here("outputs", "TravTime_FeaBut_summary.csv"), row.names=FALSE)
