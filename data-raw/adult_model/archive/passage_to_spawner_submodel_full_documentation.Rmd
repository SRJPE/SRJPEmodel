---
title: "Passage to Spawner Submodel Full documentation"
...
### Initial Adult Upstream Passage and Prespawn Mortality Analysis
...
# Passage to Spawner (P2S) Model: Predicting Chinook Salmon Spawner Abundance from Upstream Passage Estimates in the Sacramento River Valley {.tabset}
...
```{r, message = FALSE, warning = FALSE, eval = FALSE}
battle_P2S_results <- fit_passage_to_spawner_model(observed_adult_input,
                                                   adult_model_covariates,
                                                   "battle creek",
                                                   "wy_type")

clear_P2S_results <- fit_passage_to_spawner_model(observed_adult_input,
                                                  adult_model_covariates,
                                                  "clear creek",
                                                  "wy_type")

deer_P2S_results <- fit_passage_to_spawner_model(observed_adult_input,
                                                 adult_model_covariates,
                                                 "deer creek",
                                                 "wy_type")

mill_P2S_results <- fit_passage_to_spawner_model(observed_adult_input,
                                                 adult_model_covariates,
                                                 "mill creek",
                                                 "wy_type")
...
We then save the P2S model fits and full `stanfit` results as data objects:
```{r, message = FALSE, warning = FALSE, eval = FALSE}
usethis::use_data(P2S_model_fits, overwrite = TRUE)
usethis::use_data(battle_P2S_results, overwrite = TRUE)
usethis::use_data(clear_P2S_results, overwrite = TRUE)
usethis::use_data(deer_P2S_results, overwrite = TRUE)
usethis::use_data(mill_P2S_results, overwrite = TRUE)
```

Finally, we can compare all covariates for all streams and save the object for diagnostics:

```{r, message = FALSE, echo = FALSE, warning = FALSE}
knitr::kable(head(comparison_results, 10))
```
...
### Individual Spawner Counts by Stream

```{r, echo = FALSE, message = FALSE, warning = FALSE}
predicted_vs_observed_spawner_counts <- P2S_model_fits |>
  filter(str_detect(par_names, "predicted_spawner")) |>
  rename(pred_spawner_count = mean) |> 
  left_join(observed_adult_input_wide |>
              mutate(obsv_spawner_count = ifelse(stream == "deer creek", holding_count, redd_count)) |>
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
predicted_vs_observed_spawner_counts |>
...
observed_adult_input_wide |>
  ...
  ggplot(aes(x = year, y = carcass_estimate)) +
  geom_line() +
...
observed_adult_input_wide |>
  ...
  ggplot(aes(x = year, y = upstream_estimate)) +
  geom_line() +
  xlab("Year") + ylab("Spawner estimate") +
...
all_data_sources <- P2S_model_fits |>
  ...
  mutate(data_type = "P2S spawners estimates",
         ...
         lcl = `2.5%`,
         adult_count = `50%`) |>
  bind_rows(observed_adult_input_wide |>
              filter(stream %in% c("yuba river", "butte creek", "feather river")) |>
              mutate(adult_count = ifelse(stream == "yuba river", upstream_estimate, carcass_estimate),
                     data_type = ifelse(stream == "yuba river", "Passage estimate", "CJS spawner estimate"))) |>
  mutate(stream = str_to_title(stream))
...
all_data_sources |>
  ...
  scale_color_manual("Data type", values = wes_palette("GrandBudapest1")) +
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
# in adult_model_diagnostics.R as plot_conversion_rate
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
# in adult_model_diagnostics.R as josh plot
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
# in adult_model_diagnostics.R as alternative_forecast
# TODO figure out dimensions for this plot
...
predicted_vs_observed_spawner_counts |>
  ...
  ggtitle("Predicted vs. Observed Spawner Counts")
```
...
P2S_model_fits |>
  ...
  scale_color_manual(name = "Parameter name",
                     ...
                     values = wes_palette("GrandBudapest1")[2:3]) +
  facet_wrap(~stream, scales = "free") +
  ggtitle("Random Effects vs. Fixed Effects") +
...
table_4 <- P2S_model_fits |>
  ...
  mutate(par_names = case_when(par_names == "log_mean_redds_per_spawner" ~ "$log(\\mu_{\\delta})$",
                               ...
                               par_names == "R2_fixed" ~ "$R^{2}$ of fixed effects"),
         stream = str_to_title(stream),
         mean = ifelse(par_names == "log_mean_redds_per_spawner", round(exp(mean), 3), round(mean, 3)),
...
## Appendix B: diagnostics for STAN model {.tabset}
...
battle_P2S_results <- SRJPEmodel::battle_P2S_results

# TODO keep?
chains <- rstan::extract(battle_P2S_results$full_object) # see all chains

mcmc_pairs(battle_P2S_results$full_object, pars = c("b1_survival", "mean_redds_per_spawner"))
mcmc_trace(battle_P2S_results$full_object, pars = c("b1_survival", "mean_redds_per_spawner"))
mcmc_areas(battle_P2S_results$full_object, pars = c("b1_survival", "mean_redds_per_spawner"))
...
```{r, echo = FALSE, message = FALSE, warning = FALSE, eval = FALSE}
clear_P2S_results <- SRJPEmodel::clear_P2S_results

mcmc_pairs(clear_P2S_results$full_object, pars = c("b1_survival", "mean_redds_per_spawner"))
mcmc_trace(clear_P2S_results$full_object, pars = c("b1_survival", "mean_redds_per_spawner"))
mcmc_areas(clear_P2S_results$full_object, pars = c("b1_survival", "mean_redds_per_spawner"))
mcmc_rhat(rhat(clear_P2S_results$full_object))
```

### Deer

```{r, echo = FALSE, message = FALSE, warning = FALSE, eval = FALSE}
deer_P2S_results <- SRJPEmodel::deer_P2S_results

mcmc_pairs(deer_P2S_results$full_object, pars = c("b1_survival", "mean_redds_per_spawner"))
mcmc_trace(deer_P2S_results$full_object, pars = c("b1_survival", "mean_redds_per_spawner"))
mcmc_areas(deer_P2S_results$full_object, pars = c("b1_survival", "mean_redds_per_spawner"))
mcmc_rhat(rhat(deer_P2S_results$full_object))
```

### Mill
TODO this is not looking good

```{r, echo = FALSE, message = FALSE, warning = FALSE, eval = FALSE, eval = FALSE}
mill_P2S_results <- SRJPEmodel::mill_P2S_results

mcmc_pairs(mill_P2S_results$full_object, pars = c("b1_survival", "mean_redds_per_spawner"))
mcmc_trace(mill_P2S_results$full_object, pars = c("b1_survival", "mean_redds_per_spawner"))
mcmc_areas(mill_P2S_results$full_object, pars = c("b1_survival", "mean_redds_per_spawner"))
mcmc_rhat(rhat(mill_P2S_results$full_object))
```
