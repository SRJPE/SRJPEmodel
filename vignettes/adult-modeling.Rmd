...
```{r}
# TODO pull in feather carcass estimates and plot
```
...
read.csv(here::here("data-raw", "adult_model", "adult_data_all_streams.csv")) |> 
  ...
  geom_line() +
  scale_fill_manual("Spawner count method", values = c("CJS applied to carcass survey", "Spawners modeled via STAN", "Upstream Passage Estimate")) +
  xlab("Year") + ylab("Spawner count") +
  ggtitle("Spawner counts by method") +
  scale_color_discrete(name = "Spawner count method", 
                       labels = c("CJS applied to carcass survey", "Spawners modeled via STAN", "Upstream Passage Estimate")) +
  theme_minimal() +
  facet_wrap(~stream, scales = "free")
```
