...
The JPE requires an estimate of number of spawning adults for each tributary ideally with associated uncertainty. These estimates will be passed into the spawn-recruit submodel which connects spawning adults to juvenile production within each tributary.
...
* Butte Creek, and the Feather River: carcass surveys used in approach 2.

Further communication with stream teams led us to consider approach 3 for the Yuba River.
...
We only looked for univariate models (i.e. did not consider interaction) in the linear regressions due to limited data: several streams had a sample size of less than `10` which was not enough statistical power to fit interaction terms. 
...
```{r}
# TODO update these with the right pars
...
Yuba River provided adult spawner abundance as estimated by CJS. These data are only available for four years: 2014, 2015, 2019, and 2020. Following input from stream team members for Yuba, upstream passage estimates and their modeled uncertainty (see SOURCE) were recommended to be used as the primary data source for spawner abundance. 
