...
*Figure 1: Conceptual Diagram of SRJPE model. BTSPAS X sub-model shown in yellow.*
...
TODO - Add description of CUT function / Bugs & Stan divide
...
![](images/dag_diagram_screenshot.png)

# Model inputs

The model takes in three datasets:

-   **Weekly Juvenile Abundance Catch Data** Weekly catch, flow, and log normal priors needed to run the model. This is in the `SRJPEdata::weekly_juvenile_abundance_catch_data` data object.
-   **Weekly Juvenile Abundance Efficiency Data** Weekly efficiency needed to run the model. This is in the `SRJPEdata::weekly_juvenile_abundance_efficiency_data` data object.
...
```{r, eval = FALSE}
bt_spas_x_results <- run_single_bt_spas_x(bt_spas_x_bayes_params = 
...
bt_spas_x_multiple_system_results <- run_multiple_bt_spas_x(bt_spas_x_bayes_params = 
                                            SRJPEmodel::bt_spas_x_bayes_params,
                                         weekly_juvenile_abundance_catch_data = 
                                           SRJPEdata::weekly_juvenile_abundance_catch_data,
                                         weekly_juvenile_abundance_efficiency_data = 
...
There are a few functions within SRJPEmodel that can be used to view BTSPAS-X outputs and create some simple diagnostic plots.

```{r, eval = F}
# TODO check that these are working
# This function takes in a WinBUGS fit object and extracts parameter estimates in a clean tibble.
outputs <- SRJPEmodel::get_summary_table(model_fit_object = bt_spas_x_results,
                                         site = "ubc", 
                                         run_year = 2004,
                                         lifestage = "smolt", 
                                         # weeks_fit = , 
                                         sites_fit = "ubc",
                                         model_name = "2004 Battle Creek Juvenile Abundance")
# TODO create a table output of this

# This function returns a plot showing weekly estimated abundance from the model (with confidence intervals) compared to lincoln-peterson estimates
plot <- plot_juvenile_abundance(bt_spas_x_results_clean = bt_spas_x_results)

# TODO add screenshot of plot
```
