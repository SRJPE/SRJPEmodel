...
The model takes in one dataset produced by `prepare_inseason_inputs()` function. This function takes in a database connection to the model run database, a stream, a site, the covariate you would like to use to fit the model (see `?prepare_inseason_inputs()` for the full list of covariates), and a boolean value indicating if you would like to apply autocorrelation to the data.
...
inseason_fit <-  get_most_recent_model_objects(con, 
                                        ...
                                        model_name = "inseason", 
                                        stream = "clear creek")
inseason_fit_subset <- inseason_fit$`1433_inseason_lcc_clear creek_NA`
```
